section {* Refinement Kleene Algebra with Tests *}

text {* In the future this will be build directly on the KAT libraries. *}
...
text {* First we derive two characteristic properties. *}
...
text {* Next we derive refinement laws step by step. *}
...
proof - 
  ...
  hence "H p (if v then (R (t v \<cdot> t p) q) else (R (n v \<cdot> t p) q) fi) q"
    by (simp add: local.H_cond local.n_mult_comm)
 thus ?thesis
    by (rule R2)
qed
