...
theory VC_KAD_scratch
  imports Main GCD Binomial (*"$ISABELLE_HOME/src/HOL/Eisbach/Eisbach"*)

begin

subsection {* KAD: Definitions and Basic Properties *}
...
(*  by (smt add_commute add_assoc add_idem distrib_left distrib_right less_eq_def mult_onel star_inductr star_unfoldr mult_assoc) *)  
proof - 
...
lemma euclid_diff: 
  ...
  by (metis gcd.commute gcd_diff1_nat le_cases le_imp_less_or_eq)
...
lemma maximum: 
  ...
  by (auto simp: max_def rel_aka.dom_op_def rel_ad_def)
...
lemma integer_division: 
  ...
  by (rule rel_aka.wp_while_inv_break, auto simp: rel_aka.dom_op_def rel_ad_def)
...
lemma imp_reverse:
  ...
  by (rule rel_aka.wp_while_inv_break, auto, metis append.simps append_assoc hd_Cons_tl rev.simps(2))
*)
...
section {* Refinement Component Based on KAD *}

subsection{* Definition of RKAD *}
