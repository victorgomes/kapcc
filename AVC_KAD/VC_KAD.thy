...
imports "$AFP/KAD//Modal_Kleene_Algebra_Models" "../P2S2R"      
...
subsection {* Additional Facts for Antidomain Kleene Algebras *}
...
subsection {* Syntax for While Programs *}
...
subsection {* Basic w(l)p Calculus *}

text {* In the setting of modal Kleene algebras, the wlp operator is the forward modal box operator 
...
lemma fbox_export3: "|x \<cdot> ad p] q = |x] (d p + d q)"
  using local.a_de_morgan_var_3 local.ds.ddual.mult_assoc local.fbox_def by auto
...
lemma fbox_while_unfold: "|while t do x od] p = (d t + d p) \<cdot> (ad t + |x] |while t do x od] p)"
  by (metis fbox_export1 fbox_export3 local.dka.dom_add_closed local.fbox_star_unfold_var while_def)

lemma fbox_while_var1: "d t \<cdot> |while t do x od] p = d t \<cdot> |x] |while t do x od] p"
  by (metis fbox_while_unfold local.a_mult_add local.ads_d_def local.dka.dns5 local.ds.ddual.mult_assoc local.join.sup_commute)
...
proof - 
  have "ad t \<cdot> |while t do x od] p = ad t \<cdot> (d t + d p) \<cdot> (ad t + |x] |while t do x od] p)"
    by (metis fbox_while_unfold local.ds.ddual.mult_assoc)
  also have "... =  ad t \<cdot> d p \<cdot> (ad t + |x] |while t do x od] p)"
    by (metis local.a_de_morgan_var_3 local.addual.ars_r_def local.dka.dom_add_closed local.s4)
  also have "... \<le> d p \<cdot> (ad t + |x] |while t do x od] p)"
    using local.a_subid_aux1' local.mult_isor by blast
  finally show ?thesis
    by (metis local.a_de_morgan_var_3 local.a_mult_idem local.addual.ars_r_def local.ans4 local.dka.dsr5 local.dpdz.domain_1'' local.dual_order.trans local.fbox_def)
qed
...
proof -
  ...
  also have "... \<le> |while t inv i do x od] q"
    using a2 local.dka.dom_iso local.fbox_iso by fastforce
  finally show ?thesis
...
lemma fbox_frame: "d p \<cdot> x \<le> x \<cdot> d p \<Longrightarrow> d q \<le> |x] t \<Longrightarrow> d p \<cdot> d q \<le> |x] (d p \<cdot> d t)"    
  using local.dual.mult_isol_var local.fbox_add1 local.fbox_demodalisation3 local.fbox_simp by auto

lemma fbox_frame_var: "d p \<le> |x] p \<Longrightarrow> d q \<le> |x] t \<Longrightarrow> d p \<cdot> d q \<le> |x] (d p \<cdot> d t)"
  using fbox_frame local.fbox_demodalisation3 local.fbox_simp by auto   
...
lemma impl_prop: "\<lceil>P\<rceil> \<subseteq> \<lceil>Q\<rceil> \<longleftrightarrow> (\<forall>s. P s \<longrightarrow>  Q s)"
  by (auto simp: p2r_def)

lemma impl_prop_var: "rdom \<lceil>P\<rceil> \<subseteq> rdom \<lceil>Q\<rceil> \<longleftrightarrow> (\<forall>s. P s \<longrightarrow>  Q s)"
  by (simp add: impl_prop)
