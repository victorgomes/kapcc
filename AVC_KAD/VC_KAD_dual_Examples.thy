...
imports VC_KAD_dual GCD Binomial (*"$ISABELLE_HOME/src/HOL/Eisbach/Eisbach"*)
...
lemma euclid:
  ...
  by (rule rel_modal_kleene_algebra.bdia_whilei, simp_all, auto simp: p2r_def rel_ad_def gcd_non_0_nat)
...
lemma euclid_diff: 
  ...
  apply (simp_all add: p2r_def rel_ar_def) 
  apply auto[2]
  apply (safe, simp add: gcd_diff1_nat)
  using gcd_commute_nat gcd_diff1_nat by auto
...
lemma imp_reverse:
  ...
  apply (rule rel_modal_kleene_algebra.bdia_whilei_break, simp_all, simp_all add: p2r_def)
  apply auto[2]
